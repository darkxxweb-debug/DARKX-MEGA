...
import { resolveChannelJids } from './lib/channelJids.js';
import { setPairingSocket, clearPairingSocket } from './lib/webPair.js';
store.readFromFile();
...
const phoneNumber = config.pairingNumber || config.ownerNumber || "255775710774";
...
global.botname = config.botName || "DARKX-MD";
global.themeemoji = "•";
...
async function startQasimDev() {
    try {
        ...
        if (pairingCode && !isRegistered) {
            ...
            // Make this socket available to the DARKX MD pairing website
            // (lib/server.js -> /pair) so a user can type their number there
            // and receive a live pairing code, instead of pasting a session id.
            setPairingSocket(QasimDev);
            const doPairing = async (num, attempt = 1) => {
                const phoneNumberInput = num.replace(/[^0-9]/g, '');
                ...
                try {
                    let code = await QasimDev.requestPairingCode(phoneNumberInput, config.customPairingCode || 'DARKXXMD');
            ...
            // Backwards-compatible: if a number was supplied via env/config
            // (headless deployments), pair automatically like before.
            const presetNumber = config.pairingNumber || process.env.PAIRING_NUMBER;
            if (presetNumber) {
                setTimeout(() => doPairing(presetNumber), 3000);
            }
            else if (rl && !rlClosed) {
                // Local/TTY use still supports typing the number in the terminal.
                question(chalk.bgBlack(chalk.greenBright(`Type your WhatsApp number to pair now, or press Enter to use the DARKX MD pairing website instead: `))).then((answer) => {
                    if (answer && answer.trim()) {
                        doPairing(answer.trim());
                    }
                    else {
                        printLog('info', `Open the DARKX MD pairing website at http://localhost:${config.port}/pair to link your number.`);
                        if (rl && !rlClosed) {
                            rl.close();
                            rl = null;
                        }
                ...
                printLog('info', `Open the DARKX MD pairing website at http://localhost:${config.port}/pair to link your WhatsApp number.`);
            }
        }
        ...
        QasimDev.ev.on('connection.update', async (s) => {
            ...
            if (connection === "open") {
                ...
                try {
                    const jids = await resolveChannelJids(QasimDev);
                    if (jids.length) {
                        printLog('info', `Channel JID system: ${jids.map(j => j.jid).join(', ')}`);
                    }
                }
                catch (e) {
                    printLog('error', `Channel JID resolution failed: ${e.message}`);
                }
                const ghostMode = await store.getSetting('global', 'stealthMode');
                if (ghostMode && ghostMode.enabled) {
                ...
                try {
                    ...
                    await QasimDev.sendMessage(botNumber, {
                        ...
                        contextInfo: {
                            ...
                            isForwarded: true,
                            forwardedNewsletterMessageInfo: {
                                newsletterJid: global.primaryChannelJid || '120363426285893376@newsletter',
                ...
                printLog('info', `[ ${config.botName || 'DARKX-MD'} ]`);
...
async function main() {
    ...
    printLog('info', 'Starting DARKX MD BOT...');
