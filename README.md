...
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=DARKX-MD&fontSize=72&fontColor=fff&animation=twinkling&fontAlignY=32&desc=High%20Performance%20WhatsApp%20Bot&descAlignY=55&descSize=20" width="100%"/>
...
[![Version](https://img.shields.io/badge/Version-6.0.0-blue?style=for-the-badge&logo=github)](https://github.com/DARKX-MD/DARKX-MD)
...
[![Stars](https://img.shields.io/github/stars/DARKX-MD/DARKX-MD?style=for-the-badge&logo=starship&color=gold)](https://github.com/DARKX-MD/DARKX-MD/stargazers)
[![Forks](https://img.shields.io/github/forks/DARKX-MD/DARKX-MD?style=for-the-badge&logo=git&color=orange)](https://github.com/DARKX-MD/DARKX-MD/network/members)
...
```bash
git clone https://github.com/DARKX-MD/DARKX-MD.git
cd DARKX-MD
npm install
cp sample.env .env
...
```env
SESSION_ID=DARKX-MD/DARKX-MD_xxxxxxxxxxxxxxxxxxxxxxxx
```
...
```env
# ── REQUIRED (choose one) ────────────────────────────────────
SESSION_ID=DARKX-MD/DARKX-MD_your_gist_id_here
# OR
PAIRING_NUMBER=923001234567
...
```bash
sudo bash <(curl -fsSL https://raw.githubusercontent.com/DARKX-MD/DARKX-MD/main/lib/install.sh)
```
> [!IMPORTANT]
...
```bash
# After install:
nano /root/DARKX-MD/.env
cd /root/DARKX-MD && pm2 start dist/index.js --name darkx-md
...
tmux new -s darkx-md    # Start new session
npm start

# Detach:     Ctrl+B → D
# Re-attach:  tmux attach -t darkx-md
# List:       tmux ls
# Kill:       tmux kill-session -t darkx-md
```
...
# Keep alive with PM2
npm install -g pm2
pm2 start dist/index.js --name darkx-md
pm2 save && pm2 startup
```
...
| Command | Description |
|---|---|
| `pm2 logs darkx-md` | Live logs |
| `pm2 restart darkx-md` | Restart |
| `pm2 stop darkx-md` | Stop |
...
2. Paste: `https://github.com/DARKX-MD/DARKX-MD`
3. Open **Secrets** tab (🔒) and add:

   | Key | Value |
   |---|---|
   | `SESSION_ID` | `DARKX-MD/DARKX-MD_your_gist_id` |
...
**One-line Deployer:**
```bash
bash <(curl -s https://raw.githubusercontent.com/DARKX-MD/DARKX-MD/main/lib/heroku.sh)
```
**Manual:**
```bash
heroku login
...
heroku config:set SESSION_ID=DARKX-MD/DARKX-MD_your_gist_id
...
**One-line Deployer:**
```bash
bash <(curl -s https://raw.githubusercontent.com/DARKX-MD/DARKX-MD/main/lib/fly.sh)
```
**Manual:**
```bash
...
fly secrets set SESSION_ID=DARKX-MD/DARKX-MD_your_gist_id
...
```bash
# Build image
docker build -t darkx-md .

# Run
docker run -d \
  -e SESSION_ID=DARKX-MD/DARKX-MD_your_gist_id \
...
```env
# MongoDB
MONGO_URL=mongodb+srv://user:password@cluster.mongodb.net/darkxmd

# PostgreSQL
POSTGRES_URL=postgresql://user:password@host:5432/darkxmd

# MySQL
MYSQL_URL=mysql://user:password@host:3306/darkxmd
...
| `BOT_NAME` | ❌ | `DARKX-MD` | Bot display name |
| `BOT_OWNER` | ❌ | `DARKX-MD` | Owner display name |
| `PACKNAME` | ❌ | `DARKX-MD` | Sticker pack name |
...
export default {
    ...
    async handler(sock: any, message: any, args: any[], context: any = {}) {
        const {
            ...
            channelInfo       // DARKX-MD branding spread
        } = context;
...
> [!IMPORTANT]
> - Verify `SESSION_ID` starts with `DARKX-MD/DARKX-MD_`
...
[![GitHub Issues](https://img.shields.io/badge/GitHub_Issues-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/DARKX-MD/DARKX-MD/issues)
...
[MIT License](LICENSE) · Made with ❤️ by **DARKX-MD** · [DARKX-MD](https://github.com/DARKX-MD)
